...
(a) Use **{ggplot}** to make a bar plot containing the total <s>count</s> data points(?) for each `Season`.  
There may need to be some clarification about what counts is reffering to.
...
Second, you will also notice that there is a `,` on the second declaration of `Var1`. Whereas this orphan comma often breaks functions that are looking for you passing other arguments in that position, this comma does not cause a problem for `mutate()`.
