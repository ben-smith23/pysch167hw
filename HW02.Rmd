...
Code snippets are useful for creating shortcuts for task you repeat. Copy the snippet below and then go to `Tools -> Edit Code Snippets` and paste the snippet. Save. 
...
For this snippet, you only have to do it once, so the recommendation is to type the snippet hotstring `proj_dir` at the `R` console prompt (typically located in bottom left of RStudio). When you start typing, RStudio will likely start to auto-populate options and one that will appear is `proj_dir   {snippet}`. Select it an run that code in the console.
...
(I assume students are somewhat familiar with file systems and directories, but I have read kids are becoming less and less competent with them, so a brief explaination in class may be useful.)
